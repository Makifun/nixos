...
let
  backrestPort = 9898;
  backrestBase = "/playma/playma/backrest";
...
in
{
  ...
  # Rendered at runtime with real secret values — written to /playma (LUKS-encrypted XFS).
  ...
  systemd.tmpfiles.rules = [
    ...
    "d '/playma/restore'        0755 root root - -"
  ];
  ...
  # Bind on all interfaces so ligma's Traefik can proxy backrest-playma.makifun.se.
  virtualisation.oci-containers.containers.backrest = {
    ...
    volumes = [
      ...
      "/playma/playma:/playma/playma:ro"
      "/playma/dumps:/playma/dumps:ro"
      "/playma/restore:/playma/restore"
    ];
  };
