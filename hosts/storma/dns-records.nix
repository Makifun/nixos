...
#
# Each app .nix sets:
#   storma.dnsRecords."foo.makifun.se".value = "<ip>";
#
# Prerequisites (one-time, manual):
#   1. Generate key:
#      Technitium WebGUI → Settings → TSIG → Add → Name: storma-key → Save Settings
#      (Key is auto generated)
#   2. Technitium WebGUI → Zones → makifun.se → TSIG Keys: add storma-key
#   3. Enable "Allow Dynamic Updates" on the zone, restrict to storma-key TSIG
#   4. Add SOPS secret:
#        sops hosts/storma/secrets.yaml
...
in
{
  options.storma.dnsRecords = lib.mkOption {
