...
let
  backrestPort = 9898;
  backrestBase = "/storma/storma/backrest";
...
in
{
  ...
  # Rendered at runtime with real secret values — written to /storma (LUKS-encrypted XFS).
  ...
  systemd.tmpfiles.rules = [
    ...
    "d '/storma/restore'          0755 root root - -"
  ];
  ...
  # Bind on all interfaces so ligma's Traefik can proxy backrest-storma.makifun.se.
