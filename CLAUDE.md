...
| `technitium.nix` | Technitium DNS Server | 53 (DNS tcp+udp), 5380 (web console, loopback) | Primary LAN DNS resolver. **Host networking, not bridge+port-publish** — with `dns_enabled=true` in `modules/podman.nix`, netavark reserves host port 53 globally for aardvark-dns across every Podman network, so any container's `-p 53:53` fails with `listen tcp4 :53: bind: address already in use` regardless of network/IP (every other port publishes fine — confirmed by testing `-p 8280:8280` on the same image). Host networking bypasses netavark's port-publish path entirely. Web console restricted to loopback via `DNS_SERVER_WEB_SERVICE_LOCAL_ADDRESSES=127.0.0.1` (no per-container port isolation with host networking), reached through Traefik + Authentik forwardAuth (two-router pattern, like watchyourlan). Firewall restricts DNS query source to `10.10.10.0/24` + `10.10.11.0/24` (WG). `DNS_SERVER_ADMIN_PASSWORD_FILE` only read on first boot (no `/etc/dns/config.json` yet) — later UI password changes persist. SOPS: `technitium-admin-password`. Point OPNsense DHCP/clients at ligma (`10.10.10.13`) for DNS once configured. |
