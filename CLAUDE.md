...
| `pgbackrest.nix` | pgBackRest PostgreSQL backups | — | WAL archiving + physical backups to Garage `pgbackrest` bucket. Stanza `bofa`. PostgreSQL WAL archiving enabled (`archive_mode=on`, `archive_timeout=60`). Weekly full backup (Sun 01:00 UTC), daily incremental (Mon–Sat 01:00 UTC). `podman exec --env-file /run/secrets-rendered/pgbackrest.env timescaledb pgbackrest --stanza=bofa`. SOPS: `pgbackrest-s3-key`, `pgbackrest-s3-secret`. **Bootstrap (run once after first deploy):** `podman exec garage /garage bucket create pgbackrest && podman exec garage /garage key create pgbackrest-key && podman exec garage /garage bucket allow pgbackrest --read --write --owner --key pgbackrest-key`. Note the key ID and secret, add to `sops hosts/bofa/secrets.yaml`. Then: `podman exec --env-file /run/secrets-rendered/pgbackrest.env timescaledb pgbackrest --stanza=bofa stanza-create`. Run first full backup: `podman exec --env-file /run/secrets-rendered/pgbackrest.env timescaledb pgbackrest --stanza=bofa backup --type=full`. |
