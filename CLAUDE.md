...
| `monitoring.nix` | Prometheus + Grafana | 9090, 9100, 3000 | 30d retention. Grafana OIDC via Authentik, role from `groups` claim. Alerting: WiFi connected (UniFi), Authentik login success/failure → Gotify webhook. SOPS: `grafana-secret-key`, `grafana-oauth-secret`, `grafana-gotify-token`. |
