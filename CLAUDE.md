...
| `netalertx.nix` | NetAlertX network presence monitor | 20211 (UI) | ARP-scan based device tracking; notifies Gotify on new/returning devices. Host networking + NET_ADMIN/NET_RAW caps required for ARP. First-run config via UI: Settings → Plugins → GOTIFY; Settings → Scan → SUBNETS → `10.10.10.0/24, ens18`. |
