...
| `technitium.nix` | Technitium DNS Server | 53 (DNS tcp+udp), 5380 (web console, loopback) | Primary LAN DNS resolver. Port 53 published on all interfaces; firewall restricts source to `10.10.10.0/24` + `10.10.11.0/24` (WG). Web console behind Traefik + Authentik forwardAuth (two-router pattern, like watchyourlan). `DNS_SERVER_ADMIN_PASSWORD_FILE` only read on first boot (no `/etc/dns/config.json` yet) — later UI password changes persist. SOPS: `technitium-admin-password`. Point OPNsense DHCP/clients at ligma (`10.10.10.13`) for DNS once configured. |
