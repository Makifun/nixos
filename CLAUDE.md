...
**Repos moved from the personal `makifun` account to the `homelab` org (2026-09).** Forgejo repo transfers move git remotes and keep old URLs redirecting, but do **not** move the associated container registry packages — images already pushed under `git.makifun.se/makifun/<repo>` stay there permanently; only a fresh CI push lands a new image under `git.makifun.se/homelab/<repo>`. `sugma`'s Flux `GitRepository` (`sugma/k8s/bootstrap/gitrepository.yaml`) and `makiplex-bot`'s `go.mod`/`compose.yaml` were repointed to `homelab/`, but `sugma/k8s/apps/makiplex-bot/deployment.yaml` still pins the old `makifun/makiplex-bot:20260513-2d856024` image on purpose — that tag does not exist under `homelab/makiplex-bot` yet. Push to `makiplex-bot`'s `main` (or manually re-run `.forgejo/workflows/ko-build-push.yaml`, which now correctly derives `homelab/makiplex-bot` from `github.repository`) to publish a new image under the new path, then repoint the deployment.
