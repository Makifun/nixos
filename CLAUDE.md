...
| `beszel-server.nix` + `common/beszel-agent.nix` | Beszel monitoring hub + agent | 8095 (hub, loopback), 45876 (agent) | Agents self-register via universal token, routed through the existing `beszel.makifun.se` Traefik route (no NixOS/router change needed — Authentik's `skip_path_regex` for beszel already exempts `/api/*`). See "Beszel monitoring" below. |
