...
| `unifi.nix` | UniFi Network Application + webhook receiver | 8443 (UI), 8080 (inform), 3478/udp (STUN), 10001/udp (discovery), 5141/udp (syslog in), 8765 (webhook) | Two Podman containers (`unifi_network`): MongoDB 8 + linuxserver/unifi. Syslog on 5141 ingested by Alloy → Loki (job=ligma-unifi). Webhook receiver on 8765 receives UniFi event POSTs → Gotify directly (stateless, no Grafana alerting). UI self-signed cert — Traefik uses `insecureSkipVerify`. PUID/PGID=1000, MEM_LIMIT=1024M. SOPS: `unifi-webhook-gotify-token`. Configure in UniFi UI: Settings → Notifications → Webhooks, URL: `http://10.89.0.1:8765/webhook`. |
