...
| `technitium.nix` | Technitium DNS Server (external) | — | Traefik proxy to Technitium LXC at `10.10.10.3`. `technitium.makifun.se` → `https://10.10.10.3:53443` (self-signed, `insecureSkipVerify`) behind Authentik forwardAuth + OIDC. `doh.makifun.se` → `http://10.10.10.3` (no auth — DoH clients must reach it unauthenticated). OIDC client configured in `ligma/authentik/technitium.tf`. |
